                ...
                <div class="container-fluid text-monospace">
                        ...
                        <div class="card">
                                <div class="card-header bg-primary">
                                        <h1 class='text-center font-weight-bolder text-light'>Lista de préstamos</h1>
                                </div>
                                                        ...
                                                        <table class="table table-striped table-hover table-dark">
                                                                <thead class="bg-info text-light">
                                                                        <tr>
                                                                                ...
                                                                                <th class="text-center align-middle">Fecha de Término</th>
                                                                                <c:if test="${tipo == 'Administrador'}">
                                                                ...
                                                                <tbody>
                                                                        <c:forEach var="prestamos" items="${listaPrestamos}">
                                                                            <tr>
                                                                                    <c:forEach var="usuarios" items="${listaUsuarios}">
                                                                                        <c:if test="${prestamos.entidad.idUsuario == usuarios.entidad.idUsuario}">
                                                                                            <td class="text-center align-middle"><c:out value="${usuarios.entidad.nombre} ${usuarios.entidad.paterno} ${usuarios.entidad.materno}" /></td>
                                                                                        </c:if>
                                                                                    </c:forEach>
                                                                                    <c:forEach var="libros" items="${listaLibros}">
                                                                                        <c:if test="${prestamos.entidad.idLibro == libros.entidad.idLibro}">
                                                                                            <td class="text-center align-middle"><c:out value="${libros.entidad.titulo}" /></td>
                                                                                        </c:if>
                                                                                    </c:forEach>
                                                                                    <td class="text-center align-middle"><c:out value="${prestamos.entidad.fechaInicio}" /></td>
                                                                                    <td class="text-center align-middle"><c:out value="${prestamos.entidad.fechaTermino}" /></td>
                                                                                    <c:if test="${tipo == 'Administrador'}">
                                                                                    <td class="text-center align-middle">
                                                                                            <a href="mostrarPrestamo.htm?idU=<c:out value="${prestamos.entidad.idUsuario}" />&idL=<c:out value="${prestamos.entidad.idLibro}" />" class="btn btn-outline-success" title="Ver prestamo"><i class="fas fa-address-card"></i></a>
                                                                                            <a href="actualizarPrestamo.htm?idU=<c:out value="${prestamos.entidad.idUsuario}" />&idL=<c:out value="${prestamos.entidad.idLibro}" />" class="btn btn-outline-warning" title="Actualizar prestamo"><i class="fa fa-edit"></i></a>
                                                                                                <a href="eliminarPrestamo.htm?idU=<c:out value="${prestamos.entidad.idUsuario}" />&idL=<c:out value="${prestamos.entidad.idLibro}" />" class="btn btn-outline-danger" title="Eliminar prestamo" onclick="return eliminar()"><i class="fa fa-trash"></i></a>
                                                                                    </td>
                                                                                    </c:if>
                                                                            </tr>
                                                                        </c:forEach>
